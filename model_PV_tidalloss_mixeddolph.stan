...
transformed parameters{
  ...
  for (t in 2:N){
    ...
    // x[t] = x[t-1]*(alpha+gamma*tide_height_frac[t-1]) + N_dolphins[t-1]*beta+N_dolphins[t-1]*p[t-1]*psi;    // dolphin DNA sources and sinks  
    
    x[t] = x[t-1]*(alpha+gamma*tide_height_frac[t-1]) + N_dolphins[t-1]*(beta+p[t-1]*psi) + w[t-1];
